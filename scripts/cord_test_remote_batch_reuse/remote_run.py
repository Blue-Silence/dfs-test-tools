...
def remote_run(conf):
    ...
    for idx, host in enumerate(conf.REMOTE_NODES):
        ...
        for _ in range(0, conf.PARALLELISM):
            thread = RemoteProgramThread(f"cd {remote_dir} && {remote_dir}/{conf.TEST_PROGRAM} {conf.TEST_NAME} {remote_dir}/{conf.TEST_CONF} {tag} {total_parallel} {remote_dir}/OUT {conf.REUSE_INIT}", input_s, blocking_output_1, barrier_1, blocking_output_2, barrier_2, host, conf.USERNAME)
